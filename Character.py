...
class Character():
  ...
  def ai(self, player, obstacle_tiles, screen_scroll, fireball_image):
    ...
    #Move enemy and let them attack
    if self.alive:
      if not self.stunned:
        #move towards player
        ...
        if self.boss:
          if dist < 500:
            if pygame.time.get_ticks() - self.last_attack >= fireball_cooldown:
              fireball = Weapons.Fireball(fireball_image, self.rect.centerx, self.rect.centery, player.rect.centerx, player.rect.centery)
              self.last_attack = pygame.time.get_ticks()
      ...
      #When player Stunned timer runs out
